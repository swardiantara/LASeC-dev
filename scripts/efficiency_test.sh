...
for embedding in "${embeddings[@]}"; do
    for size in "${sample_sizes[@]}"; do
        timeout 10001 python -m src.lasec --dataset_type full --dataset BGL --sample_size "$size" --model agglomerative --embedding "$embedding" --threshold 0.09 --output_dir efficiency-test
    done
done
